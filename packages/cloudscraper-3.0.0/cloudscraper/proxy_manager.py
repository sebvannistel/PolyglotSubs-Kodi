...
class ProxyManager:
    ...
    def __init__(
        self, proxies=None, proxy_rotation_strategy="sequential", ban_time=300
    ):
        """
        ...
        self.banned_proxies = {}
        self.proxy_stats = defaultdict(
            lambda: {"success": 0, "failure": 0, "last_used": 0}
        )
        ...
        logging.debug(
            f"ProxyManager initialized with {len(self.proxies)} proxies using '{proxy_rotation_strategy}' strategy"
        )
    ...
    def get_proxy(self):
        ...
        available_proxies = [
            ...
            or time.time() - self.banned_proxies[p] > self.ban_time
        ]

        if not available_proxies:
            logging.warning(
                "All proxies are currently banned. Using the least recently banned one."
            )
