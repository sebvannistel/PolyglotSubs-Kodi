...
class CloudflareTurnstile:
        ...
        try:
            return (
                ...
                and (
                    ...
                    or re.search(
                        r'data-sitekey="[0-9A-Za-z]{40}"', resp.text, re.M | re.S
                    )
                )
            )
    ...
    def extract_turnstile_data(self, resp):
        try:
            ...
            return {"site_key": site_key.group(1), "form_action": form_action_url}

        except Exception as e:
            logging.error(f"Error extracting Cloudflare Turnstile data: {str(e)}")
            raise CloudflareTurnstileError(
                f"Error extracting Cloudflare Turnstile data: {str(e)}"
            )
        ...
        try:
            ...
            if challenge_response.status_code == 403:
                raise CloudflareSolveError(
                    "Failed to solve Cloudflare Turnstile challenge"
                )
            ...
            raise CloudflareTurnstileError(
                f"Error handling Cloudflare Turnstile challenge: {str(e)}"
            )
