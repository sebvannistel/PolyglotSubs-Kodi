        ...
        try:
            ...
            try:
                ...
                if "bytecode" in str(version_error).lower():
                    logging.warning(
                        f"js2py version compatibility issue detected: {version_error}"
                    )
                    logging.warning(
                        "Attempting to continue with basic js2py functionality..."
                    )
                else:
                    raise version_error
            ...
            if "sys" in str(e) and "not associated with a value" in str(e):
                logging.error(
                    "js2py sys variable conflict detected. This may be due to a js2py version issue."
                )
                # Try a fallback approach
                try:
                    # Create a completely isolated context
                    isolated_context = js2py.EvalJs()
                    isolated_context.execute(
                    ...
                    logging.error(
                        f"Fallback js2py evaluation also failed: {str(fallback_error)}"
                    )
            raise
