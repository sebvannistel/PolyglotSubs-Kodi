...
class CloudflareV3:
        ...
        try:
            return (
                ...
                and (
                    ...
                    re.search(r"window\._cf_chl_ctx\s*=", resp.text, re.M | re.S)
                    or
    ...
    def extract_v3_challenge_data(self, resp):
        try:
            ...
            challenge_ctx = re.search(
                r"window\._cf_chl_ctx\s*=\s*({.*?});", resp.text, re.DOTALL
            )
            ...
            challenge_opt = re.search(
                r"window\._cf_chl_opt\s*=\s*({.*?});", resp.text, re.DOTALL
            )
            ...
            raise CloudflareChallengeError(
                f"Error extracting Cloudflare v3 challenge data: {str(e)}"
            )
        ...
        try:
            ...
            try:
                ...
                return (
                    ...
                    else self.generate_fallback_response(challenge_data)
                )

            except Exception as js_error:
                logging.warning(
                    f"JavaScript execution failed: {str(js_error)}, using fallback"
                )
                return self.generate_fallback_response(challenge_data)
        ...
        try:
            ...
            for field_match in re.finditer(
                ...
                if field_name not in [
                    "jschl_answer"
                ]:  # Don't include the answer field yet
            ...
            raise CloudflareChallengeError(
                f"Error generating v3 challenge payload: {str(e)}"
            )
    ...
    def handle_V3_Challenge(self, resp, **kwargs):
        try:
            ...
            challenge_answer = self.execute_vm_challenge(
                challenge_info, url_parsed.netloc
            )

            # Generate the challenge payload
            payload = self.generate_v3_challenge_payload(
                challenge_info, resp, challenge_answer
            )
            ...
            if not challenge_url.startswith("http"):
                challenge_url = (
                    f"{url_parsed.scheme}://{url_parsed.netloc}{challenge_url}"
                )
            ...
            raise CloudflareChallengeError(
                f"Error handling Cloudflare v3 challenge: {str(e)}"
            )
