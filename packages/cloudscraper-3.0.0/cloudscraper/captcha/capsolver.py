    ...
    def checkErrorStatus(response, fnct):
        if response.status_code in [500, 502]:
            raise CaptchaServiceUnavailable(
                f"CapSolver: Server Side Error {response.status_code}"
            )
        ...
        if (
            rPayload.get("errorDescription", False)
            and "Current system busy" not in rPayload["errorDescription"]
        ):
    ...
    def requestJob(self, jobID):
        ...
        response = polling2.poll(
            lambda: self.session.post(
                ...
                json={"clientKey": self.api_key, "taskId": jobID},
                timeout=30,
            ),
        ...
        raise CaptchaTimeout("CapSolver: Error failed to solve Captcha.")
