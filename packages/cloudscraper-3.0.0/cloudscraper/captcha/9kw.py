...
class captchaSolver(Captcha):
    ...
    def __init__(self):
        ...
        self.captchaType = {"reCaptcha": "recaptchav2", "hCaptcha": "hcaptcha"}
    ...
    def requestJob(self, jobID):
        if not jobID:
            raise CaptchaBadJobID("9kw: Error bad job id to request against.")
    ...
    def getCaptchaAnswer(self, captchaType, url, siteKey, captchaParams):
        ...
        if captchaType not in self.captchaType:
            raise CaptchaException(
                f"9kw: {captchaType} is not supported by this provider."
            )
