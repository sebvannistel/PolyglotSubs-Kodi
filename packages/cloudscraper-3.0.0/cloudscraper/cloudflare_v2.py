...
class CloudflareV2:
    ...
    def extract_challenge_data(self, resp):
        try:
            ...
            challenge_data = re.search(
                r"window\._cf_chl_opt=({.*?});", resp.text, re.DOTALL
            )
            ...
            raise CloudflareChallengeError(
                f"Error extracting Cloudflare challenge data: {str(e)}"
            )
    ...
    def generate_challenge_payload(self, challenge_data, resp):
        try:
            ...
            raise CloudflareChallengeError(
                f"Error generating Cloudflare challenge payload: {str(e)}"
            )
    ...
    def handle_V2_Challenge(self, resp, **kwargs):
        try:
            ...
            payload = self.generate_challenge_payload(
                challenge_info["challenge_data"], resp
            )
            ...
            raise CloudflareChallengeError(
                f"Error handling Cloudflare v2 challenge: {str(e)}"
            )
    ...
    def handle_V2_Captcha_Challenge(self, resp, **kwargs):
        try:
            ...
            captcha_response = Captcha.dynamicImport(
                ...
                "hCaptcha", resp.url, site_key.group(1), self.cloudscraper.captcha
            )
            ...
            if challenge_response.status_code == 403:
                raise CloudflareSolveError(
                    "Failed to solve Cloudflare v2 captcha challenge"
                )
            ...
            raise CloudflareCaptchaError(
                f"Error handling Cloudflare v2 captcha challenge: {str(e)}"
            )
