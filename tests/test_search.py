...
class TestSanitizeResults(unittest.TestCase):
    def test_sanitize_preserves_translate_options(self):
        ...
        found_german_subtitlecat = any(
            r["service_name"] == "subtitlecat" and r["lang"] == "de"
            for r in sanitized_results
        )
        found_chinese_subtitlecat = any(
            r["service_name"] == "subtitlecat" and r["lang"] == "zh"
            for r in sanitized_results
        )
        found_french_subtitlecat = any(
            r["service_name"] == "subtitlecat" and r["lang"] == "fr"
            for r in sanitized_results
        )
        found_english_opensubtitles = any(
            r["service_name"] == "opensubtitles" and r["lang"] == "en"
            for r in sanitized_results
        )

        self.assertTrue(
            found_german_subtitlecat, "German Subtitlecat translate option missing"
        )
        self.assertTrue(
            found_chinese_subtitlecat, "Chinese Subtitlecat translate option missing"
        )
        self.assertTrue(
            found_french_subtitlecat,
            "French Subtitlecat translate option (SHARED_TRANSLATION_CONTENT) missing",
        )
        self.assertTrue(
            found_english_opensubtitles, "English OpenSubtitles option missing"
        )
