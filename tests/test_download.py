...
class TestDownloadFunction(unittest.TestCase):
    ...
    @patch(
        "a4kSubtitles.download.__copy_sub_local"
    )  # Patching this as it's not relevant to the core logic
    ...
    @patch(
        "a4kSubtitles.download.__insert_lang_code_in_filename",
        side_effect=lambda core, filename, lang_code: (
            f"{filename.rsplit('.', 1)[0]}.{lang_code}.{filename.rsplit('.', 1)[1]}"
            if "." in filename
            else f"{filename}.{lang_code}"
        ),
    )
    def test_download_with_save_callback(
        self,
        mock_insert_fn,
        mock_download_internal,
        mock_extract_gzip,
        mock_extract_zip,
        mock_postprocess,
        mock_copy_sub_local,
        ...
        core_mock.utils.slugify_filename.side_effect = (
            lambda x: x
        )  # Keep filename as is
        core_mock.utils.get_lang_id.return_value = "eng"  # For 'en' language
        core_mock.os.path.join.side_effect = lambda *args: "/".join(
            args
        )  # Simple path join mock
