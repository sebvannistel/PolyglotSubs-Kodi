...
def __query_service(core, service_name, meta, request, results):
    ...
    try:
        ...
        if response and response.status_code == 200 and response.text:
            service_results = service.parse_search_response(
                core, service_name, meta, response
            )
        else:
            service_results = []
...
def __get_last_results(core, meta):
    ...
    try:
        ...
        has_bsplayer_results_expired = (
            core.time.time() - last_results["timestamp"] > 3 * 60
        )
        if has_bsplayer_results and has_bsplayer_results_expired:
            last_results["results"] = list(
                filter(
                    lambda r: r["service_name"] != "bsplayer", last_results["results"]
                )
            )
...
def __sanitize_results(core, meta, results):
    ...
    for result in results:
        ...
        unique_key_components = [
            result["service_name"],
            result["lang"],
            result["action_args"].get(
                "filename", result.get("name", "")
            ),  # Use filename from action_args or result name
        ]

        if not key_url and (
            result["action_args"].get("needs_client_side_translation")
            or result["action_args"].get("method_type") == "SHARED_TRANSLATION_CONTENT"
            or result["action_args"].get("save_callback")
        ):
            ...
            if result["service_name"] == "subtitlecat":
                if result["action_args"].get("needs_client_side_translation"):
                    unique_key_components.append(
                        result["action_args"].get("original_srt_url")
                    )
                elif (
                    result["action_args"].get("method_type")
                    == "SHARED_TRANSLATION_CONTENT"
                ):
                    unique_key_components.append(
                        result["action_args"].get("detail_url")
                    )  # detail_url is more stable for shared
...
def __prepare_results(core, meta, results):
    ...
    def sorter(x):
        ...
        x["action_args"].setdefault(
            "episodeid", meta.episode.zfill(3) if meta.episode else ""
        )
        ...
        if meta.is_tvshow:
            ...
            if matching_offset == 0:
                matching_offset = _match_numbers(
                    cleaned_file_nameparts, cleaned_nameparts
                )
        ...
        return (
            ...
            -(
                core.difflib.SequenceMatcher(
                    None, cleaned_file_nameparts, cleaned_nameparts
                ).ratio()
                + matching_offset
            ),
...
def __parse_languages(core, languages):
    ...
    return list(
        {
            language
            for language in (core.kodi.parse_language(x) for x in languages)
    ...
    for auth_thread, search_thread in request_threads:
        if not auth_thread:
            ...
            thread = core.threading.Thread(
                target=__chain_auth_and_search_threads,
                args=(core, auth_thread, search_thread),
            )
...
def __search(core, service_name, meta, results):
    ...
    core.logger.debug(
        lambda: "%s - %s"
        % (service_name, core.json.dumps(requests, default=lambda o: "", indent=2))
    )

    threads = []
    for request in requests:
        thread = core.threading.Thread(
            target=__query_service, args=(core, service_name, meta, request, results)
        )
        threads.append(thread)
...
def search(core, params):
    ...
    meta.languages = __parse_languages(
        core, core.utils.unquote(params["languages"]).split(",")
    )
    ...
    for service_name in core.services:
        if len(results) > 0 and (
            __has_results(service_name, results) or service_name not in force_search
        ):
            continue
        ...
        if auth_request:
            auth_thread = core.threading.Thread(
                target=__auth_service, args=(core, service_name, auth_request)
            )

        search_thread = core.threading.Thread(
            target=__search, args=(core, service_name, meta, results)
        )
