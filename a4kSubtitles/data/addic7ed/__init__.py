...
from a4kSubtitles.lib import kodi, request, utils
...
if (
    not utils.os.path.exists(__remote_tvshows_path)
    or utils.os.path.getmtime(__remote_tvshows_path) < __embedded_tvshows_mtime
):
...
def __download_data(url, etag, destpath):
    response = request.execute(
        utils.core,
        {"method": "GET", "url": url, "headers": {"If-None-Match": etag}},
        progress=False,
    )
...
utils.core.threading.Thread(
    target=__download_data,
    args=(__github_tvshows_url, __tvshows_etag, __remote_tvshows_path),
).start()
