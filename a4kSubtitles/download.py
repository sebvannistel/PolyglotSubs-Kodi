...
def __extract_zip(core, archivepath, filename, episodeid):
    ...
    try:
        ...
        namelist = [
            file.decode(core.utils.default_encoding) if core.utils.py2 else file
            for file in files
        ]

    subfile = core.utils.find_file_in_archive(
        core, namelist, sub_exts + sub_exts_secondary, episodeid
    )
    ...
    try:
        ...
        if core.kodi.get_bool_setting("general.use_chardet"):
            ...
            if core.utils.py3:
                ...
                detected_lang_code = core.utils.get_lang_id(
                    detection["language"], core.kodi.xbmc.ISO_639_2
                )
                if detection["confidence"] == 1.0 or detected_lang_code == lang_code:
                    encoding = detection["encoding"]

            if not encoding:
                encoding = core.utils.code_pages.get(
                    lang_code, core.utils.default_encoding
                )
...
def __copy_sub_local(core, subfile):
    ...
    media_name = core.os.path.splitext(
        core.os.path.basename(core.kodi.xbmc.getInfoLabel("Player.Filename"))
    )[0]
    ...
    if core.kodi.get_kodi_setting("subtitles.storagemode") == 0:
        ...
        file_dest = core.os.path.join(
            folder_dest, ".".join([media_name, lang_code, extension])
        )
...
def download(core, params):
    ...
    archivepath = core.os.path.join(
        core.utils.temp_dir, "sub.zip"
    )  # Used if actual archive download
    ...
    if cb:  # If there's a save_callback, it's responsible for saving the final SRT.
        ...
        filepath = core.os.path.join(
            core.utils.temp_dir, filename
        )  # Target path for the final SRT
        ...
        __download(
            core, filepath, request
        )  # Standard download to archivepath (filepath)
    ...
    core.kodi.xbmcplugin.addDirectoryItem(
        handle=core.handle, url=filepath, listitem=listitem, isFolder=False
    )
