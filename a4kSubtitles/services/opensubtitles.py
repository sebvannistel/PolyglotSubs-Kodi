...
def build_auth_request(core, service_name):
    ...
    if token_cache is not None and "ttl" in token_cache:
        token_ttl = core.datetime.fromtimestamp(
            core.time.mktime(core.time.strptime(token_cache["ttl"], __date_format))
        )
...
def parse_auth_response(core, service_name, response):
    ...
    if response.status_code == 400:
        core.kodi.notification(
            "OpenSubtitles authentication failed! Bad username. Make sure you have entered your username and not your email in the username field."
        )
        return
...
def parse_search_response(core, service_name, meta, response):
    ...
    def map_result(result):
        ...
        if (
            ...
            and imdb_id != meta.imdb_id_as_int
        ):
            return None

        filename = result["files"][0]["file_name"]
        language = core.utils.get_lang_id(
            result["language"], core.kodi.xbmc.ENGLISH_NAME
        )
...
def build_download_request(core, service_name, args):
    ...
    def download_request(response):
        ...
        return {"method": "GET", "url": result["link"], "stream": True}
