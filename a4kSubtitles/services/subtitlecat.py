    ...
    with _COUNTER_LOCK:
        ...
        if (now - _LAST_THROTTLE_RESET_TIME) > GOOGLE_API_COUNTER_RESET_INTERVAL_SECONDS:
            if GOOGLE_API_REQUEST_COUNT > 0:  # Only log if it was active
                core.logger.debug(
                    f"[{service_name}] gtranslate: {log_prefix_throttle}"
                    f"Resetting Google API request counter (was {GOOGLE_API_REQUEST_COUNT}) "
        ...
        try:
            ...
            if (response_json and response_json[0] and isinstance(response_json[0], list) and
                ...
                if _get_setting(core, "debug", False):
                    core.logger.debug(
                        f"[{service_name}] gtranslate: {log_prefix_single}"
                        f"Malformed/empty segment for '{line_to_translate[:30]}...'. "
                        f"Response: {str(response_json)[:100]}. "
                        f"Full text: {r_text_for_debug[:200]}"
                    )
        ...
        try:
            ...
            if len(translated_lines_after_split) == len(lines_to_translate):
                ...
                if _get_setting(core, "debug", False):
                    core.logger.debug(
                        f"[{service_name}] gtranslate: {log_prefix}"
                        f"Newline-joined batch of {len(lines_to_translate)} lines translated successfully "
                        "in one API call. Split count matched."
                    )
                ...
                core.logger.warning(
                    f"[{service_name}] gtranslate: {log_prefix}"
                    f"Newline-joined batch returned {len(translated_lines_after_split)} lines, "
                    f"expected {len(lines_to_translate)}. Response JSON[0] preview: "
                    f"{str(response_json[0])[:150] if response_json and response_json[0] else 'N/A'}."
                )
                if recursion_depth == 0:  # Only trigger full per-line fallback if this is the first attempt for the chunk
                    ...
                    if lines_to_process_concurrently_map:
                        ...
                        core.logger.debug(
                            f"[{service_name}] gtranslate: {log_prefix}"
                            f"Submitting {len(lines_to_process_concurrently_map)} single lines to ThreadPoolExecutor "
                            f"for fallback (max_workers={max_workers})."
                        )
                        future_to_idx = {}
