...
def parse_search_response(core, service_name, meta, response):
    ...
    def map_result(result):
        ...
        for release_name in result["custom_releases"]:
            similarity = core.difflib.SequenceMatcher(
                None, release_name, meta.filename_without_ext
            ).ratio()
        ...
        return {
            ...
            "sync": (
                "true"
                if meta.filename_without_ext in result["custom_releases"]
                else "false"
            ),
...
def build_download_request(core, service_name, args):
    ...
    def retry_download(response):
        if response.status_code >= 500:
            return {"method": "GET", "url": args["url"]}
